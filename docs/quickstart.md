...
![Welcome dialog](assets/images/quickstart-welcome.png)
...
1. Open the **Manage Xenia** page (sidebar → Manage Xenia).
...
!!! tip
    Leave **automatic update checks** on (default). The Manager will badge the Manage Xenia page when a newer Xenia build is available. Details in [Manage Xenia](guides/manage-xenia.md#updates).
...
![Scan directory](assets/images/library-scan.png)

Supported inputs include disc images and extracted formats handled by the built-in file parsers (ISO, XEX, STFS, SVOD, ZAR). GPD/XDBF files hold achievements/profile data, not launchable games. If a game shows as "Unknown Game", see [Library](guides/library.md#unknown-games) and [Troubleshooting](troubleshooting.md).
