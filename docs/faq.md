...
Disc images and extracted Xbox 360 formats handled by the built-in parsers (ISO, XEX, STFS/SVOD containers, ZAR archives). GPD/XDBF files hold achievements/profile data, not launchable games. If a file will not parse here, Xenia itself would not boot it either — fix the dump. See [Library](guides/library.md#scanning-and-adding-games).
