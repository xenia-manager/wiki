...
1. Confirm at least one variant shows an **installed version** on the [Manage Xenia page](guides/manage-xenia.md). A game assigned to an uninstalled variant fails immediately.
...
2. Confirm the patch is **enabled** in the [Patches dialog](guides/patches.md#patches-dialog-configurator) and the Title/Media IDs match your dump.
...
!!! tip
    Before filing, delete nothing: keep the failing `xenia.log`, the Manager log covering the same timeframe, and (for content/patch issues) a screenshot or listing of the Content Viewer / Patches dialog state.
