...
```
XeniaManager.BigScreen.exe
Config/            # config.json (+ config.json.backup), games.json (library), dashboard-settings.json
...
Each `Emulators/<Variant>/` also contains `portable.txt` (enables portable mode), `config/`, `content/`, and `patches/` created on install; `screenshots/`, `xenia.log`, `gamecontrollerdb.txt`, and `xconfig.settings` appear at runtime. See [Manage Xenia](guides/manage-xenia.md#emulator-folder-layout).
