...
![Xenia Settings](../assets/images/xenia-settings.png)
...
| **Per-game** (this page) | The game's config file (`Emulators/<variant>/config/<game_title>.config.toml` | Only that title; overrides global values |
...
1. Pick the emulator variant or game at the top (Canary / Mousehook / Netplay). The form rebuilds for that build's config keys.
...
![Optimized Settings](../assets/images/xenia-settings-optimized.png)
...
- `Emulators/<Variant>/<name>.config.toml` — live file Xenia actually reads (e.g. `xenia-canary.config.toml`).
- `Emulators/<Variant>/config/<name>.config.toml` — stock copy the Manager edits/stores.
- `Emulators/<Variant>/config/<GameTitle>.config.toml` — per-game file, swapped into the live path at launch.
...
- **Settings page is empty or errors** — the variant is probably not installed or its config file is missing/corrupt. Reinstall or Repair that variant on the [Manage Xenia page](manage-xenia.md).
