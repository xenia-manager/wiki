...
The Manage Xenia page installs, updates, and repairs the actual emulator builds. Xenia Manager itself is only a launcher — the games run inside one of the Xenia variants installed here.

![Manage Xenia](../assets/images/manage-xenia.png)
...
Emulators/
  Xenia Canary/
    xenia_canary.exe
    xenia-canary.config.toml      # live file Xenia reads (Manager copies stock/per-game file here at launch)
    portable.txt                  # enables portable mode (created on install)
    config/                       # stock copy (<name>.config.toml) + per-game files (<GameTitle>.config.toml)
    content/                      # DLC/TU (or shared Emulators/Content/ when unified — see below)
    patches/                      # enabled .patch.toml files for this variant
    screenshots/                  # Xenia's own screenshots (runtime-generated)
    xenia.log                     # per-emulator log (runtime-generated)
    gamecontrollerdb.txt          # SDL controller mappings (auto-refreshed)
    xconfig.settings              # emulator-generated settings
  Xenia Mousehook/
...
Only `portable.txt`, `config/`, `content/`, and `patches/` are created on install — the rest appear when Xenia first runs or downloads mappings.

!!! warning
    Do not rename executables or move files out of these folders by hand. The Manager locates the emulator by these exact paths. If you must intervene, use the repair/reinstall actions on the Manage Xenia page.
...
- **Automatic checks**: when update checking is enabled (see [Manager Settings](manager-settings.md#updates)), the Manager compares your installed version against the version manifest on startup and badges the Manage Xenia page when an update is available (`update_available` flag + `last_update_check_date` per variant).
- **Manual check**: use the Check-for-updates action on the Manage Xenia page.
